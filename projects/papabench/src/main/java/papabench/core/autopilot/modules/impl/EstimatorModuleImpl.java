...
public class EstimatorModuleImpl implements Estimator, Module {
	...
	private final Position3D speed;

	/* rotational speed in radians per second (phi,psi,theta) */
	private final Position3D rotationalSpeed;
		
	/* flight time in seconds */
	private int flightTime = 0;

	/* horizontal ground speed in module and dir (m/s, rad (CW/North)) */
	private final HorizSpeed horizontalSpeed; // (estimator_hspeed_mod, estimator_hspeed_dir)
   

	/* Wind and airspeed estimation sent by the GCS */
	private final float windEast = 0f;
    private final float windNorth = 0f; /* m/s */
	private final float airspeed = 0f; /* m/s */
	
	/* IR related values */
	private final boolean irEstimationModeEnabled = false;
