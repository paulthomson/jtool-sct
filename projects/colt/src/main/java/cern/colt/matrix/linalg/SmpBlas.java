...
import cern.colt.EDU.oswego.cs.dl.util.concurrent.FJTask;
...
<li>Implemented using Doug Lea's fast lightweight task framework ({@link cern.colt.EDU.oswego.cs.dl.util.concurrent}) built upon Java threads, and geared for parallel computation.</li>
</ul>

@see cern.colt.EDU.oswego.cs.dl.util.concurrent.FJTaskRunnerGroup
@see cern.colt.EDU.oswego.cs.dl.util.concurrent.FJTask
...
public void dgemm(final boolean transposeA, final boolean transposeB, final double alpha, final DoubleMatrix2D A, final DoubleMatrix2D B, final double beta, final DoubleMatrix2D C) {
	...
	*/
	...
	long flops = 2L*m*n*p;
	int noOfTasks = this.maxThreads; // each thread should process at least 30000 flops
...
/**
 * Prints various snapshot statistics to System.out; Simply delegates to {@link cern.colt.EDU.oswego.cs.dl.util.concurrent.FJTaskRunnerGroup#stats}.
 */
public void stats() {
