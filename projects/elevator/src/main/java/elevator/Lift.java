...
class Lift extends Thread {
  ...
  public static int count = 0;
    ...
    private final int firstFloor, lastFloor;
    ...
    public Lift(final int numFloors, final Controls c) {
        super("Lift " + count++);
        controls = c;
        firstFloor = 1;
        lastFloor = numFloors;
    ...
    private void addPeople(final Vector people) {
        System.out.println(getName() + " picking up " + people.size() +
                " passengers on " + currentFloor);
        for (final Enumeration e = people.elements(); e.hasMoreElements(); ) {
            final int toFloor = ((Integer) e.nextElement()).intValue();
            peopleFor[toFloor] += 1;
        }
    }
}
